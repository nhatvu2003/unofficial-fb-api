...
const LEVELS: LogLevel[] = ["error", "warn", "success", "info", "debug"];

function getLogLevel(): number {
  const env = process?.env ?? {};
  const lvl = (env.NVFCA_LOG_LEVEL ?? "info").toLowerCase() as LogLevel;
...
class Logger {
  ...
  static log(...args: any[]): void {
    if (!Logger.isConsoleEnabled() || LOG_LEVEL_IDX < 3) return;
    console.log(colorize("info", "[INFO]"), ...args);
  }

  static error(...args: any[]): void {
    if (!Logger.isConsoleEnabled() || LOG_LEVEL_IDX < 0) return;
    console.error(colorize("error", "[ERROR]"), ...args);
  }

  static warn(...args: any[]): void {
    if (!Logger.isConsoleEnabled() || LOG_LEVEL_IDX < 1) return;
    console.warn(colorize("warn", "[WARNING]"), ...args);
  }

  static success(...args: any[]): void {
    if (!Logger.isConsoleEnabled() || LOG_LEVEL_IDX < 2) return;
    console.log(colorize("success", "[SUCCESS]"), ...args);
  }

  static debug(...args: any[]): void {
    if (!Logger.isConsoleEnabled() || LOG_LEVEL_IDX < 4) return;
    if (
  ...
  static httpFile(...args: any[]): void {
    try {
      const enabled =
        process?.env?.NVFCA_HTTP_VERBOSE === "1" ||
