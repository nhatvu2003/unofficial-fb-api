...
export class ConfigValidator {
    ...
    static validate(config) {
        if (!config || typeof config !== "object") {
            throw ErrorHandler.handleValidationError("Configuration must be a valid object", { providedConfig: config });
        }
        // Merge with defaults
        const mergedConfig = {
            ...this.DEFAULT_CONFIG,
        ...
        if (typeof mergedConfig.timeout !== "number" || mergedConfig.timeout <= 0) {
            throw ErrorHandler.handleValidationError("timeout must be a positive number", { value: mergedConfig.timeout });
        }
        if (typeof mergedConfig.retryAttempts !== "number" ||
            mergedConfig.retryAttempts < 0) {
            throw ErrorHandler.handleValidationError("retryAttempts must be a non-negative number", { value: mergedConfig.retryAttempts });
        }
