 ...
 */
function isCookieJar(obj: any): obj is CookieJar {
  if (!obj) return false;
  try {
    // Prefer instanceof check when possible (covers tough-cookie instances)
    if (obj instanceof CookieJar) return true;
  } catch (e) {
...
export default class JarAdapter {
  static wrap(inputJar?: any): any {
    ...
    if (isCookieJar(inputJar)) {
      const jar: any = inputJar;
      return {
        _tough: jar,
        getCookiesSync: (url: string) => jar.getCookiesSync(url),
        getCookies: (url: string) => Promise.resolve(jar.getCookiesSync(url)),
        getCookieStringSync: (url: string) => jar.getCookieStringSync(url),
        getCookieString: (url: string) =>
          Promise.resolve(jar.getCookieStringSync(url)),
        setCookieSync: (cookieStr: string, url: string) =>
          jar.setCookieSync ? jar.setCookieSync(cookieStr, url) : undefined,
    ...
    const maybeJar: any = inputJar as any;
    const tough =
      maybeJar._tough || (isCookieJar(maybeJar) ? maybeJar : undefined);
    const adapter: any = {
      _tough: tough,
      getCookiesSync:
        typeof maybeJar.getCookiesSync === "function"
          ? maybeJar.getCookiesSync.bind(maybeJar)
          : (url: string) =>
              maybeJar.getCookies ? maybeJar.getCookies(url) : [],
      getCookies:
        typeof maybeJar.getCookies === "function"
          ? maybeJar.getCookies.bind(maybeJar)
          : (url: string) => Promise.resolve(adapter.getCookiesSync(url)),
      getCookieStringSync:
        typeof maybeJar.getCookieStringSync === "function"
          ? maybeJar.getCookieStringSync.bind(maybeJar)
          : (url: string) =>
              maybeJar.getCookieString ? maybeJar.getCookieString(url) : "",
      getCookieString:
        typeof maybeJar.getCookieString === "function"
          ? maybeJar.getCookieString.bind(maybeJar)
          : (url: string) => Promise.resolve(adapter.getCookieStringSync(url)),
      setCookieSync:
        typeof maybeJar.setCookieSync === "function"
