...
class HttpClient {
  ...
  static proxyAgent: unknown | null = null;
   ...
   */
  private static getAgent(): any | undefined {
   ...
   */
  private static normalizeObjectValues(obj?: Record<string, any>): void {
    if (!obj) return;
    for (const k in obj) {
      if (
   ...
   */
  static setProxy(proxy: string | null) {
    // https-proxy-agent may export different shapes across versions; construct defensively
    HttpClient.proxyAgent = proxy ? new (HttpsProxyAgent as any)(proxy) : null;
  }
    ...
    return {
      ...
      setCookie(cookieStr: string, url: string) {
        ...
        try {
          ...
          (toughJar as any).setCookie &&
            (toughJar as any).setCookie(cookieStr, url, () => {});
        } catch {
          try {
            (toughJar as any).setCookieSync &&
              (toughJar as any).setCookieSync(cookieStr, url);
          } catch {}
        }
      },
  ...
  static isReadableStream(obj: unknown): boolean {
    return (
      ...
      HttpClient.getType((obj as any)._readableState) === "Object"
    );
  }
   ...
   */
  static async cleanGet(url: string): Promise<HttpResponse> {
