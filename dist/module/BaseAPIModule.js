...
export class BaseAPIModule {
                    ...
                    if (parsed) {
                        ...
                        if (attempt && typeof attempt.then === 'function') {
                            attempt.catch(() => {
                                ...
                                try {
                                    this.setCookieSafe(`${key}=${value}; Path=${path}`, messengerUrl);
                                }
                                catch (e) { /* ignore */ }
                            });
                        }
                        ...
                        try {
                            this.setCookieSafe(stripped, "https://www.messenger.com");
                        }
                        catch (e) { /* ignore */ }
                    }
                }
        ...
        try {
            ...
            try {
                if (this.config && this.config.developmentLog)
                    fs.appendFileSync('./dist/test/cookie_set_errors.log', `syncKeyCookiesToMessenger error: ${String(e)}\n`);
            }
            catch { }
        }
        return res;
     ...
     */
    syncKeyCookiesToMessenger() {
        try {
            const jar = this.ctx && this.ctx.cookieJar;
            if (!jar)
                return;
            ...
            try {
                if (typeof jar.getCookieStringSync === 'function')
                    cookieStrFB = jar.getCookieStringSync('https://www.facebook.com') || '';
                else if (jar._tough && typeof jar._tough.getCookieString === 'function')
                    jar._tough.getCookieString('https://www.facebook.com', (e, v) => { cookieStrFB = v || ''; });
            }
            catch (e) {
                cookieStrFB = '';
            }
            ...
            for (const key of keys) {
                ...
                if (m && m[1]) {
                    ...
                    try {
                        ...
                        try {
                            if (this.config && this.config.developmentLog)
                                fs.appendFileSync('./dist/test/cookie_set_success.log', `set ${key} ok\n`);
                        }
                        catch { }
                        ...
                        try {
                            let cookieStr = '';
                            if (typeof jar.getCookieStringSync === 'function')
                                cookieStr = jar.getCookieStringSync('https://www.messenger.com') || '';
                            else if (jar._tough && typeof jar._tough.getCookieString === 'function')
                                jar._tough.getCookieString('https://www.messenger.com', (e, v) => { cookieStr = v || ''; });
                        ...
                        try {
                            fs.appendFileSync('./dist/test/cookie_set_errors.log', `setCookieSafe error for ${key}: ${String(err)}\n`);
                        }
                        catch { }
                    }
                }
            ...
            try {
                fs.appendFileSync('./dist/test/cookie_set_errors.log', `syncKeyCookiesToMessenger outer error: ${String(e)}\n`);
            }
            catch { }
        }
    }
    parseAndCheckLogin(data, retryCount = 0) {
     ...
     */
    async makeRequest(url, form, options = {}) {
     ...
     */
    async makeGraphQLRequest(docId, variables, friendlyName) {
        const form = {
            av: this.ctx.UserID,
            doc_id: docId,
    ...
    async ensureFbDtsg() {
        try {
            if (this.ctx && this.ctx.fb_dtsg)
                return;
            const resp = await HttpClient.get('https://www.facebook.com/ajax/dtsg/?__a=true', this.ctx.cookieJar);
            if (!resp || !resp.body)
                return;
...
//# sourceMappingURL=BaseAPIModule.js.map