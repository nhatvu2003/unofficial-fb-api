...
export class ErrorHandler {
    ...
    static handleHttpError(error, context) {
        if (error.response?.statusCode === 401 ||
            ...
            return this.createError(ErrorType.AUTHENTICATION, "Authentication failed - please check cookies/login status", error, context);
        }
        if (error.response?.statusCode === 429) {
            return this.createError(ErrorType.RATE_LIMIT, "Rate limit exceeded - please retry later", error, context);
        }
        if (error.code === "ENOTFOUND" || error.code === "ECONNREFUSED") {
            return this.createError(ErrorType.NETWORK, "Network connection failed", error, context);
        }
        return this.createError(ErrorType.NETWORK, `HTTP request failed: ${error.message}`, error, context);
    }
    static handleParsingError(error, context) {
        return this.createError(ErrorType.PARSING, `Failed to parse response: ${error.message}`, error, context);
    }
