...
export class MessageModule extends BaseAPIModule {
    ...
    private async executeSendMessage(
        ...
        try {
            ...
            try { fs.writeFileSync('./dist/test/last_legacy_full_response.json', JSON.stringify({ responseRaw: response, threadID, messageAndOTID }, null, 2), 'utf8'); } catch {}
        } catch (e) { /* non-fatal */ }
     ...
     */
    private async handleMessageEnhancements(
        msg: MessageOptions,
        form: { [key: string]: any }
    ): Promise<void> {
        // Handle sticker
    ...
    private async executeUnsendMessage(messageID: string): Promise<void> {
        ...
        const response = await this.makeRequest(
            FacebookConstants.ENDPOINTS.UNSEND_MESSAGE,
            form
        );
    ...
    private async executeMarkAsRead(threadID: string): Promise<void> {
    ...
    private async executeSendTypingIndicator(
        threadID: string,
        isTyping: boolean
    ): Promise<void> {
